  ...
  return (
    <ScrollView className="flex-1 bg-gray-50" showsVerticalScrollIndicator={false}>
      ...
      <View className="bg-white px-5 pt-4 pb-0 border-b border-gray-100">
        <Pressable onPress={() => router.back()} className="mb-3 p-1 self-start">
      ...
      <View className="bg-white mt-3 p-5">
        <Text className="text-base font-semibold text-navy mb-3">Patient Information</Text>
        <InfoRow label="Date of Birth" value={formatDate(patient.dateOfBirth)} />
        <InfoRow label="Gender" value={patient.sex === 'male' ? 'Male' : patient.sex === 'female' ? 'Female' : 'Other'} />
        <InfoRow label="Patient ID" value={patient.id.substring(0, 8).toUpperCase()} />
      ...
      {/* Assessment Summary */}
      <View className="bg-white mt-3 p-5">
        <Text className="text-base font-semibold text-navy mb-3">Assessment Summary</Text>
        <View className="gap-2 mb-4">
          <SummaryCard count={assessments.length} label="Assessments Total" color="#E6F7F5" textColor="#0D9E94" />
          {highRiskCount > 0 && (
            <SummaryCard count={highRiskCount} label="High Risk Findings" color="#FFEDD5" textColor="#EA580C" />
          )}
          {lowRiskCount > 0 && (
            <SummaryCard count={lowRiskCount} label="Low Risk Findings" color="#DBEAFE" textColor="#2563EB" />
          )}
        </View>
      ...
      {assessments.length > 0 && (
        <View className="bg-white mt-3 p-5">
          <View className="flex-row justify-between items-center mb-3">
            <Text className="text-base font-semibold text-navy">Recent Assessments</Text>
            <Pressable onPress={() => router.push(`/(app)/patients/${patientId}/history`)}>
            ...
            >
              ...
              <View className="flex-1">
                ...
                {assessment.bodyLocation && (
                  <Text className="text-xs text-gray-400">{assessment.bodyLocation}</Text>
                )}
              </View>
...
function InfoRow({ label, value }: { label: string; value: string }) {
  return (
    <View className="flex-row justify-between py-2 border-b border-gray-50">
      ...
      <Text className="text-sm font-medium text-navy flex-1 text-right ml-4">{value}</Text>
    </View>
  );
}
...
function SummaryCard({
  ...
  return (
    <View className="flex-row items-center rounded-xl p-3" style={{ backgroundColor: color }}>
